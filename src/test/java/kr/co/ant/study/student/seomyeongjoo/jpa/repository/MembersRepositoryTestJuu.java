...
@Import({MemberMRepository.class}) //Repository Annotation은 Jpa 관련 설정이 아니라 DataJpaTest시에 Bean으로 등록 되지 않기때문에 Import로 강제 등록
@ActiveProfiles("juu") //자신이 설정한 application-이니셜.yml을 로드 하기 위해 Profile 설정
@Rollback(false)
class MembersRepositoryTestJuu {

    @Autowired
    private MemberMRepository repository;
