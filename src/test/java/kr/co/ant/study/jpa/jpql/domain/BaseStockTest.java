...
class BaseStockTest {
	...
	void 수정() {
		...
		stock.setStockQty(new BigDecimal(100));
	...
	void 기본조회_jpql() {
		String sql = "select s " 
				+ "from BaseStock s " 
				+ "where s.initId = :id";
		BaseStock stock = manager.createQuery(sql, BaseStock.class).setParameter("id", 1L).getSingleResult();
	...
	void 조인_jpql() {
		...
		String sql2 = "select s " + "from BaseStock s " + "join BaseStockHistory h "
				+ "on s.initId = h.baseStock.initId " + "where s.initId = :id";
		BaseStock stock2 = manager.createQuery(sql2, BaseStock.class).setParameter("id", 1L).getSingleResult();
	...
	void 조인_수량이10개_이하인것_조회_jpql() {
		...
		//assertThat(stock.getBaseStockHistories().size()).isEqualTo(21);
	}
	 ...
	 */
		...
		log.info("조인 결과 :: {}", stock.getBaseStockHistories().size());
		assertThat(stock.getBaseStockHistories().size()).isEqualTo(21);
