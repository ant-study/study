...
@Slf4j
public abstract class ReflectGenericQuestion {
	 ...
	 * @return
	 */
	public abstract <T> T getValue(Object vo, String fieldName, Class<T> clazz)throws Exception;
	...
	public void startTest() throws Exception {
		...
		String id = getValue(orgi, "id", String.class);
		assertStringEquals(orgi.getId(), id);
		
		String name = getValue(orgi, "name", String.class);
		assertStringEquals(orgi.getName(), name);
		
		int age = getValue(orgi, "age", Integer.class);
		assertNumberEquals(orgi.getAge(), age);

		LocalDate brithday = getValue(orgi, "brithday", LocalDate.class);
